...
function hasUsedAIFreeTrial() {
  return (
    localStorage.getItem('katokits_free_uses') > 0 ||
    localStorage.getItem('katokits_email_signedup') === 'true'
  );
}
...
function initializeForms() {
  // AI Free Form
  const aiForm = document.getElementById('ai-free-form');
  if (aiForm) {
  ...
  if (freebieForm) {
    freebieForm.onsubmit = function (e) {
      ...
      alert(
        'Thank you! Check your email for the free Colors & Shapes Discovery Kit!'
      );
    };
  }
